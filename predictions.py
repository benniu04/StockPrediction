...
with tab_backtest:
    ...
    signals_series = pd.Series(signals, index=feat_df.index, name="Signal").shift(1).fillna(0).astype(int)

    aligned_prices = price_df["Adj Close"].reindex(signals_series.index)
...
with tab_prediction:
    ...
    st.metric("Probability(up tomorrow)", f"{meta_p*100:.2f}%")
